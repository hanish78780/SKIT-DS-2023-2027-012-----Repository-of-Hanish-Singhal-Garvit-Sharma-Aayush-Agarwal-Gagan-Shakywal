...
def generate_pdf(interval="weekly"):
    ...
    if not student_logs:
        ...
        for student_name, logs in student_logs.items():
            ...
            log_table_data.append([Paragraph(first_date, meta_cell_style), Paragraph(f"<code>{first_sha}</code>", meta_cell_style), Paragraph(html.escape(first_msg) if first_msg else "(No commit message)", msg_style), Paragraph("<b>_____ / 10</b>", marks_style)])
            for date_val, sha_val, msg_val in logs[1:]:
                log_table_data.append([Paragraph(date_val, meta_cell_style), Paragraph(f"<code>{sha_val}</code>", meta_cell_style), Paragraph(html.escape(msg_val) if msg_val else "(No commit message)", msg_style), ""])
